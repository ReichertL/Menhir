...
use_oram=True
power=[20]#enumerate(range(16,24))
...
outdir="./results/insertionDeletion-size:2^24-cols:1-retrieveExactly:"+str(retrieveExactly)+"-BlockSize:0"
maxNumThreads=1
...
def run_with_seed(s):   
    ...
    for nC in sorted(columnOptions,reverse=True):
        ...
        for nD in sorted(numDatapoints,reverse=True):
            ...
            #one oram for all datapoints, no parallelisation
            logCapacity=int(ceil(log(nD)/log(2)))
                        ...
                        for maxSelectivityValue in selectivities:
                            ...
                            for vs in valueSizes:
                                ...
                                for rangequeryrange in rangequery_ranges:
                                    ...
                                    oramLogCapacityString="--logcapacity "+str(logCapacity)+" "
